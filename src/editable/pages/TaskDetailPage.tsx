...
function BackLink({ task }: { task: TaskKey }) {
  ...
  return (
    <Link href={taskConfig?.route || '/'} className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-white/80 px-4 py-2 text-sm font-black shadow-sm backdrop-blur transition hover:-translate-y-0.5">
...
function ArticleDetail({ post, related, comments }: { post: SitePost; related: SitePost[]; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_340px] lg:px-8 lg:py-16">
      <article className="min-w-0 rounded-[2.4rem] border border-black/10 bg-[var(--detail-surface)] p-5 shadow-[0_24px_80px_rgba(17,17,17,0.08)] sm:p-8 lg:p-12">
        <BackLink task="article" />
        <p className="mt-8 text-[11px] font-black uppercase tracking-[0.28em] text-[var(--detail-accent)]">{categoryOf(post, 'Article')}</p>
        <h1 className="mt-4 max-w-4xl text-4xl font-black leading-[0.92] tracking-[-0.08em] sm:text-5xl lg:text-6xl">{post.title}</h1>
        {images[0] ? (
          <div className="mt-8 overflow-hidden rounded-[2rem] border border-black/10 bg-black/5">
  ...
  return (
    <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_380px]">
        <article className="rounded-[2.5rem] border border-black/10 bg-white p-6 shadow-[0_24px_80px_rgba(17,17,17,0.08)] sm:p-9">
          <div className="grid gap-6 sm:grid-cols-[160px_1fr]">
            <div className="flex h-40 w-40 items-center justify-center overflow-hidden rounded-[2rem] bg-[var(--detail-bg)] ring-1 ring-black/10">
            ...
            <div>
              ...
              <h1 className="mt-3 text-4xl font-black leading-[0.94] tracking-[-0.08em] sm:text-5xl lg:text-6xl">{post.title}</h1>
...
function ClassifiedDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-7 px-4 py-10 sm:px-6 lg:grid-cols-[0.82fr_1.18fr] lg:px-8 lg:py-16">
      <aside className="rounded-[2.4rem] border border-black/10 bg-black p-7 text-white shadow-[0_24px_90px_rgba(17,17,17,0.22)] lg:sticky lg:top-24 lg:self-start">
      ...
      <article className="rounded-[2.4rem] border border-black/10 bg-white p-6 shadow-[0_24px_80px_rgba(17,17,17,0.08)] sm:p-9">
  ...
  return (
    <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[0.72fr_1.28fr]">
        <aside className="rounded-[2.4rem] border border-black/10 bg-white p-7 shadow-[0_24px_80px_rgba(17,17,17,0.08)] lg:sticky lg:top-24 lg:self-start">
        ...
        <div className="columns-1 gap-5 space-y-5 md:columns-2">
          {(images.length ? images : ['/placeholder.svg?height=900&width=1200']).map((image, index) => (
            <figure key={`${image}-${index}`} className="break-inside-avoid overflow-hidden rounded-[2rem] border border-black/10 bg-white shadow-[0_18px_50px_rgba(17,17,17,0.08)]">
...
function BookmarkDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-16">
      <article className="rounded-[2.4rem] border border-black/10 bg-white p-7 shadow-[0_24px_80px_rgba(17,17,17,0.08)] sm:p-10">
        <BackLink task="sbm" />
        <div className="mt-10 flex h-20 w-20 items-center justify-center rounded-[2rem] bg-black text-white"><Bookmark className="h-9 w-9" /></div>
        ...
        {website ? <Link href={website} target="_blank" rel="noreferrer" className="mt-8 inline-flex items-center gap-2 rounded-full bg-black px-5 py-3 text-sm font-black text-white">Open saved resource <ExternalLink className="h-4 w-4" /></Link> : null}
        <BodyContent post={post} />
      </article>
      ...
      <article className="rounded-[2.4rem] border border-black/10 bg-white p-6 shadow-[0_24px_80px_rgba(17,17,17,0.08)] sm:p-9">
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-[120px_1fr]">
          ...
          <div>
            ...
            <h1 className="mt-3 text-4xl font-black leading-[0.94] tracking-[-0.08em] sm:text-6xl">{post.title}</h1>
          </div>
        </div>
        <BodyContent post={post} />
        {fileUrl ? (
          <div className="mt-8 overflow-hidden rounded-[2rem] border border-black/10 bg-[var(--detail-bg)]">
            <div className="flex items-center justify-between gap-3 border-b border-black/10 bg-white p-4">
              ...
              <Link href={fileUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full bg-black px-4 py-2 text-xs font-black text-white">Download <Download className="h-4 w-4" /></Link>
            </div>
...
function ProfileDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[420px_minmax(0,1fr)] lg:px-8 lg:py-16">
      <aside className="rounded-[2.4rem] border border-black/10 bg-white p-8 text-center shadow-[0_24px_80px_rgba(17,17,17,0.08)] lg:sticky lg:top-24 lg:self-start">
        ...
        <h1 className="mt-6 text-4xl font-black leading-[0.94] tracking-[-0.08em]">{post.title}</h1>
...
function MapBox({ src, label }: { src: string; label: string }) {
  return (
    <div className="overflow-hidden rounded-[2rem] border border-black/10 bg-white shadow-[0_18px_50px_rgba(17,17,17,0.08)]">
...
function ContactAction({ website, phone, email }: { website?: string; phone?: string; email?: string }) {
  ...
  return (
    <div className="mt-5 rounded-[2rem] border border-black/10 bg-white p-5 shadow-[0_18px_50px_rgba(17,17,17,0.08)]">
...
function BadgeLine({ label, value }: { label: string; value: string }) {
  return <div className="flex items-center justify-between gap-4 rounded-2xl border border-white/12 bg-white/8 px-4 py-3 text-sm"><span className="font-black uppercase tracking-[0.16em] opacity-60">{label}</span><span className="font-black">{value}</span></div>
}
...
function RelatedPanel({ task, post, related, compact = false }: { task: TaskKey; post: SitePost; related: SitePost[]; compact?: boolean }) {
  ...
  return (
    <aside className="min-w-0 space-y-5">
      {!compact ? (
        <div className="rounded-[2rem] border border-black/10 bg-white/80 p-5 backdrop-blur">
...
function RelatedCard({ task, post }: { task: TaskKey; post: SitePost }) {
  ...
  return (
    <Link href={buildPostUrl(task, post.slug)} className="group flex gap-3 rounded-2xl border border-black/10 bg-white p-3 transition hover:-translate-y-0.5 hover:shadow-[0_18px_40px_rgba(17,17,17,0.12)]">
...
function EditableComments({ slug, comments }: { slug: string; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <section className="mt-10 rounded-[2rem] border border-black/10 bg-white/80 p-5">
