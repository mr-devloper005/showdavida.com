...
const buttonClass =
  'inline-flex h-12 items-center justify-center rounded-full bg-[var(--sd-accent)] px-6 text-sm font-semibold text-white transition duration-200 hover:-translate-y-0.5 hover:bg-[var(--sd-accent-strong)] disabled:opacity-60'
const messageClass = 'rounded-[var(--sd-radius-sm)] px-4 py-3 text-sm font-medium'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`${messageClass} ${status === 'success' ? 'bg-emerald-500/15 text-emerald-300' : 'bg-red-500/15 text-red-300'}`}>{message}</p> : null}
